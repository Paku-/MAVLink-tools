...
def analyze_mavlink_log(logfile_path):
    ...
    field_values = defaultdict(list)   # Store values for each field in each message type
        ...
        if len(values) > 0:  # Only process fields with values
            try:
                ...
                if abs(avg_val) < 1e-5:
                    continue
                has_data = True
                output_lines.append(f"{field_key}: Count={len(values)}, Min={min_val:.2f}, Max={max_val:.2f}, Average={avg_val:.2f}\n")
            except Exception as e:
